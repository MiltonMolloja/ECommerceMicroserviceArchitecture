...
BEGIN
    ...
    DECLARE @OldAttributeId INT = (SELECT AttributeId FROM Catalog.ProductAttributes WHERE AttributeName = 'Condition');
    
    -- Eliminar valores de productos
    DELETE FROM Catalog.ProductAttributeValues WHERE AttributeId = @OldAttributeId;
    
    -- Eliminar valores del atributo
    DELETE FROM Catalog.AttributeValues WHERE AttributeId = @OldAttributeId;
    
    -- Eliminar el atributo
    DELETE FROM Catalog.ProductAttributes WHERE AttributeId = @OldAttributeId;
...
INSERT INTO Catalog.ProductAttributes (AttributeName, AttributeNameEnglish, AttributeType, IsFilterable, IsSearchable, DisplayOrder)
VALUES ('Condicion', 'Condition', 'Select', 1, 1, 2);
...
SELECT 
    av.ValueText as Condicion,
    av.ValueTextEnglish as Condition,
...
GROUP BY av.ValueText, av.ValueTextEnglish, av.DisplayOrder
